...
export class UserService {
  ...
  addManager(mgr:Manager):Observable<status>{
    return this.http.post<status>(this.baseUrl+'addmanager',mgr);
  }
  addEngineer(egr:Engineer):Observable<status>{
    return this.http.post<status>(this.baseUrl+'addengineer',egr);
  }
  addCustomer(cus:Customer){
    return this.http.post<status>(this.baseUrl+'addcustomer',cus);
  }
